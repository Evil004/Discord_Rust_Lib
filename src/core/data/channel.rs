...
use crate::core::json;
use crate::core::http::{get_from_discord_api, post_to_discord_api};
...
impl Channel {
    pub async fn get_channel(id: &str, client: &Client) -> Channel {
        let channel_data = get_from_discord_api(&format!("/channels/{}", id), client).await.expect("Error");
    ...
    pub async fn send_message(&self, message: &str, client: &Client) {
        ...
        post_to_discord_api(&format!("/channels/{}/messages", self.id), data, client).await;
    }
}